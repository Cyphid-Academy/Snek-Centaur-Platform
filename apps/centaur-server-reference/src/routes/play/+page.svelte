...
async function redeem(reference: string): Promise<boolean> {
  ...
  try {
    // Redemption hands back a grant: the working credential, and — when the
    // handoff was minted under a readable session — the first link of a
    // renewal chain. This page takes the credential and lets the chain go: a
    // round of the demo is far shorter than a credential's life, and a page
    // that has lapsed is one reload away from another.
    // spec: identity-and-authorization/token-lifetime-and-refresh#renewal-does-not-interrupt-a-live-session
    const grant = await client().action(api.issuance.redeemSignInHandoff, {
      reference,
      verifier,
    });
