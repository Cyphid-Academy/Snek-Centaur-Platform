...
Depends on: bot-framework/heuristic-vocabulary, bot-framework/scalar-discipline, global-invariants/centaur-state-boundary, team-management/archive-not-delete.
...
Preferences available to it: for each
...
lever of the framework's scalar discipline. The configuration SHALL be
team-scoped and SHALL persist for the lifetime of the team.

#### Scenario: #server-replacement-inherits-defaults
- **WHEN** a team replaces its hosting server
- **THEN** the new server serves the team's existing heuristic defaults unchanged — the defaults are the team's own state, which no server change resets or re-seeds

#### Scenario: #archiving-preserves-configuration
- **WHEN** a team is archived and later unarchived
...
Depends on: team-management/team-record, identity-and-authorization/mutation-authorization, team-management/coaches, global-invariants/team-private-centaur-state.

Only a team's current captain SHALL be able
...
parameters, and pinning. Team-scoped bot
configuration SHALL be readable by every current member and by the team's
coaches at all times, regardless of any game's
state.
...
- **THEN** the mutation is rejected — the captain is the only identity these mutations authorise
...
- **THEN** it appears in no in-game Drive affordance until its row exists
...
Depends on: bot-framework/softmax-decision, bot-framework/per-snake-portfolio.
...
sampling decision with no cache
invalidation and no restart, the result reaching the framework as the
single opaque scalar of its portfolio contract. Lower temperature SHALL bias decisions
...
- **THEN** everything is visible but every editing affordance is disabled — the affordances take their disabled state from the server-held captaincy
...
predicate accepts, dims the rest,
