...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-bot-configuration` then `openspec archive --skip-specs -y migrate-bot-configuration` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
