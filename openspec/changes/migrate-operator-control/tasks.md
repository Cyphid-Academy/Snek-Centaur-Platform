...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-operator-control` then `openspec archive --skip-specs -y migrate-operator-control` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
