...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-accounts-and-profiles` then `openspec archive --skip-specs -y migrate-accounts-and-profiles` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
