...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-team-server-management` then `openspec archive --skip-specs -y migrate-team-server-management` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
