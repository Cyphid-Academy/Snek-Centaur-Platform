...
a visitor's data access follows their own identity rather than the server
they visit; nevertheless a
malicious server operator can serve modified client code that exfiltrates
whatever data its visitors can legitimately read. The platform accepts
this trade-off: it attempts no detection or prevention, and users are
expected to sign in only on servers they trust, exactly as with any web
application. What bounds the trade-off is
that security is enforced outside the library: because no
security-relevant invariant is ever enforced by server-controlled code, a
hostile server can betray only what its own visitors could already read,
never widen anyone's read or write authority.
...
- **THEN** the team record holds no nominated server, and the team cannot enter games until a server is nominated again
...
- **THEN** it is a hosted team's credential held under that team's own scope; no credential names, authenticates, or empowers the server as an entity
...
- **THEN** resolving the nominated domain is the entire ownership check — the platform is sending to the domain, not receiving credentials from it, so delivery reaching only the domain's legitimate operator is the security property relied upon, and what a mis-delivery could ever yield stays inside a game credential's per-team, per-game scope

### Requirement: team-server-management/invitation-credential-carriage
Depends on: identity-and-authorization/game-credential-scope, global-invariants/credential-confinement.

Each game invitation SHALL carry the receiving team's per-team game credential together with the game context the server needs to act on it, and SHALL be self-sufficient: a server that has accepted needs no further provisioning exchange. The invitation is the credential's sole delivery channel.

#### Scenario: #sole-secret-channel
- **WHEN** a server's inbound platform exchanges are examined for what each must protect
- **THEN** only the invitation endpoint handles a secret at all — nomination, healthchecks, and every other exchange need no secret-handling discipline, because credential confinement gives the credential this one channel
