...
team-server-management, global-invariants** (all four actually cited;
the list is extended whenever a citation is genuinely warranted).
...
- **Scoring vocabulary is phrased abstractly.** The walkover value is
  stated as what it is — par, 1.0, the normalised-scoring value for a
  field of one — without citing the engine's scoring requirements: this
  capability states the outcome, not the formula, and what it depends on
  is that the formula have exactly one implementation
  (global-invariants/one-shared-engine, cited) rather than on the
  formula itself. game-engine is accordingly not a declared dependency.
