...
Depends on: game-lifecycle/game-record, game-lifecycle/instance-per-game, global-invariants/single-convex-deployment, rooms-and-matchmaking/team-enrolment, global-invariants/state-confined-to-owning-runtime#game-instance-holds-only-its-games-state.

A tournament SHALL consist of a configured number of rounds — one or more, fixed when the tournament is created — each round a distinct game in its own right: a full game record served by its own freshly provisioned instance. The tournament's meta-parameters — its round count, its inter-round interlude, and its scheduled start time — SHALL be properties of the tournament as a whole, never of any single round: the tournament is the one record spanning its rounds, and so lives where state outliving a single game lives. The tournament's participant set SHALL be fixed when the tournament begins — the teams then enrolled in its room — and every round SHALL be contested by that same set.

#### Scenario: #every-round-a-real-game
- **WHEN** any round of a tournament is examined
- **THEN** it is a complete game — its own record, its own instance, its own outcome — indistinguishable in kind from a standalone game, its per-game state included: a round's successor inherits nothing but what the tournament record carries
...
Every round of a tournament SHALL be played under the same gameplay configuration: the configuration captured from the tournament's opening game when the tournament was created, copied into each subsequent round's own game record. The inherited set SHALL exclude the tournament meta-parameters themselves — round count, interlude, scheduled start time — which describe the tournament, not any round's play. The captured base configuration — never a live read of the opening game — SHALL be what each round's instance is seeded with at initialisation.
...
- **THEN** the round starts anyway — the readiness gate governs manually initiated starts, and no scheduled round consults it
...
Depends on: game-lifecycle/game-record.

A team that forfeits a tournament round SHALL be scored in that round's outcome as a loss with score 0 and recorded among the round's unseated teams on the game record; that marking — not the score value — SHALL be what distinguishes a forfeit from a played loss. The forfeit SHALL be reflected wherever the round's results are consumed: outcome and ranking surfaces, leaderboards, and the finished round's replay presentation.
