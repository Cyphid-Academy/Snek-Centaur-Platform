...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-decision-transparency` then `openspec archive --skip-specs -y migrate-decision-transparency` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
