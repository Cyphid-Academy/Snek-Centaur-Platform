...
Depends on: operator-control/exclusive-selection, operator-control/board-and-move-interface.

For the operator's held snake the interface SHALL display each candidate direction's current score in two mutually consistent places: the board cells adjacent to the snake's head, coloured by score on a monotone ramp, and the score labels on the four-direction staging affordance, coloured consistently with the cells. A direction with no published score SHALL render, in both places, in a distinct neutral state visually distinguishable from every score value.
