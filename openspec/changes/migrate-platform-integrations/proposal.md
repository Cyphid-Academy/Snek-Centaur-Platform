...
None. The mint itself (Q2), which peer stories this capability may cite
(Q8 — global-invariants was added to the declaration afterwards, under the
citation principle, which is not a reopening of that decision: the
concern Q8 settled was reaching sideways into peer stories), the
full-train disposal (Q6), and the admin-only key model were settled with
the author in the Phase B synthesis; the two reconciliations this change
performs —
