...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-platform-integrations` then `openspec archive --skip-specs -y migrate-platform-integrations` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
