...
relying party exactly as a peer Cyphid system is: it obtains a human-scoped
credential through the assertion exchange it already authenticates with
under `service-principal-assertions` and `trusted-issuer-registry`, so no
second protocol is introduced.

Nothing is minted, because two requirements already forbid the alternative:
