...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-game-lifecycle` then `openspec archive --skip-specs -y migrate-game-lifecycle` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
