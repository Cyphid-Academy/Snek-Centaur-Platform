...
- [ ] 1.1 Mint `packages/app-shell` (`@cyphid/snek-app-shell`), a Svelte 5
...
- [ ] 1.2 The mounting contract: mode and affordances as explicit mount
...
- [ ] 1.3 The one state binding: a reactive read surface with connection
...
- [ ] 1.4 The one board rendering: `BoardView` and its snake-silhouette child
...
- [ ] 1.5 Repoint `apps/visual-tester` at the shared renderer — the Q-B plan
  item: the spec does not bind the dev tool, but exactly one rendering exists
  in the repository
- [ ] 1.6 The navigation shell in `apps/centaur-server-reference` — the
...
- [ ] 1.7 Add `// spec:` citations in the code written for this change, and
  `// design:` references where this change's design rationale warrants them
- [ ] 1.8 Run `pnpm spec:check` and the full battery with the implementation
