...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-rooms-and-matchmaking` then `openspec archive --skip-specs -y migrate-rooms-and-matchmaking` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
