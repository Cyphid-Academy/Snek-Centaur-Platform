...
A game's configuration SHALL comprise exactly the engine's parameter vocabulary — the parameters, ranges, defaults, and disable sentinels the engine defines — and nothing else; in particular it SHALL NOT include any parameter that configures bot behaviour, heuristics, or Drive management (bot behaviour parameters are owned elsewhere and travel their own path). Every write SHALL be validated authoritatively at the configuration record — a value outside its parameter's defined type or range is rejected there, identically for the writes of every surface.
...
A game's configuration SHALL be editable by permitted administrative users only while the game awaits launch. At launch the configuration SHALL be frozen as an immutable snapshot for the remainder of the game's life — a freeze guarded inside the transaction of every write it must exclude, so a write cannot slip past a launch committing concurrently; a game that reaches its end without ever launching likewise stops being editable.
...
Depends on: global-invariants/one-shared-engine, game-configuration/board-preview-lock-in, global-invariants/state-confined-to-owning-runtime#clients-restart-clean.

Editing a board-affecting parameter of a not-yet-launched game SHALL regenerate the board preview by running the one shared engine build's board generation platform-side against the game's current parameters — boards are only ever generated by the platform, never by a client, and the preview is therefore produced by the same generation the launch will use. Each regeneration SHALL overwrite the game's single current-preview value, which is delivered reactively to every configuration client so that all viewers render the same candidate; clients render the delivered preview and SHALL NOT run any board-generation algorithm themselves. The current-preview value holds only the latest candidate — prior candidates are not retained — and it designates the game's starting state only through lock-in or launch.
...
- **THEN** every viewer renders the same current preview from the shared platform-held value — no client holds a private candidate, and none survived the refresh
...
Depends on: game-engine/board-generation-retry, global-invariants/client-truthfulness#rejections-reach-the-user.

When board generation fails — during a preview regeneration or during launch — the engine's structured infeasibility error SHALL reach the configuring user reactively, identifying the constraint that failed so the configuration can be adjusted and retried; a launch whose board generation fails SHALL NOT proceed.
