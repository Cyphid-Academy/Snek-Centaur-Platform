...
- [ ] 2.1 On explicit author instruction, `pnpm spec:fold migrate-live-game-observation` then `openspec archive --skip-specs -y migrate-live-game-observation` at the tail of the PR that completes the implementation (fold enforces capability-dependency order)
