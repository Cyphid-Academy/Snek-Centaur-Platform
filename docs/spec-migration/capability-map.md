...
## Proposed dependency DAG (Q8)

Authoring changes declare only the dependencies their spec text actually
cites, and extend that declaration whenever a further citation is warranted;
the graph is kept acyclic (lint-enforced via each Purpose's "Depends on:"
declaration).
...
is a meta layer that capabilities declare and cite: a requirement cites a gi
invariant wherever its soundness depends on that invariant remaining true, so
the blast radius of relaxing one is traceable through its dependents (see
openspec/README.md).
