...
8. **Dependency DAG.** Approve the proposed DAG and archive order in
   ...
   they actually cite, extending the declaration when a further citation is warranted).
...
| 05-REQ-067 | Captain-designated coaches: read-only live visibility | live-game-observation | author abstractly (team-private live state) to fit the declared dependencies; Captain-designation half joins 05-REQ-068a in team-management |
...
| 06-REQ-027 | Owning server sole writer via game credential | decision-transparency | dedupe:07-REQ-004; re-author as "team's hosting server is sole writer" (credential mechanics outside the declared dependencies) |
