...
**Where new REVIEW items go**:
- **Agent-surfaced items** (Ambiguity / Contradiction / Gap / Proposed Addition) are written inline in the module file's `## REVIEW Items` section while open, and migrate to the `.review.md` at the next migration pass once resolved.
- **Human-initiated items** (Correction / Amendment per `SPEC-INSTRUCTIONS.md` §"Human-initiated items") are first-raised already-resolved and are written **directly to the module's `.review.md` decision log**, never inline. The module file's `## REVIEW Items` section stays in its single-pointer form. If a task plan tells you to add a resolved Correction/Amendment inline "for now" with migration deferred, the plan is wrong — write it straight to the `.review.md`.
