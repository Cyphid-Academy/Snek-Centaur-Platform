...
**05-REQ-012**: Convex shall permit the Captain of a Centaur Team to add and remove human members from the team, and to transfer the Captain designation to another current team member (by updating `captainUserId`). The Captain holds all team-level operator authorities, including turn submission per [06] and the in-game operator-boot affordance per [06-REQ-040b] / [08-REQ-068a] (a manual forced disconnect that severs the booted operator's session connection identically to a network disconnect, with no persistent state). (Per-operator tempo toggling per [06-REQ-040b] is *not* a Captain-only authority — every operator toggles their own tempo; see resolved 08-REVIEW-011 and 08-REVIEW-024.) These mutations shall be subject to the mid-game freeze of [03-REQ-046].
...
**05-REQ-026** *(negative)*: The game-configuration parameter set shall not include any parameter that configures bot behaviour, heuristic defaults, or Drive management. Such parameters are owned by [06] and by the Snek Centaur Server web application.
...
**05-REQ-027**: Convex shall maintain a persistent record of every game. Each game record shall capture at minimum the game's room, the bound parameter snapshot ([05-REQ-024]), a status value from the closed set `{ not-started, playing, finished }`, a reference to its SpacetimeDB instance (per [05-REQ-032]), the final outcome and scores (populated at game end per [05-REQ-038]), the timestamps at which the game entered the `playing` and `finished` states, and — for tournament games — the set of Centaur Teams that forfeited the round by invitation refusal (see [05-REQ-027a] and [03-REQ-056]).

**05-REQ-027a**: For tournament games, the game record shall include a `forfeitedTeamIds` field listing each Centaur Team whose nominated server rejected or failed to respond to the game-start invitation per [03-REQ-056]. This field is empty for non-tournament games (which abort orchestration on any refusal rather than recording forfeits) and is empty for tournament games where every server accepted. The forfeit list shall be visible in replays and reflected in scoring and leaderboard surfaces (per [08]). A forfeited team's entry in the game's outcome shall be recorded as a loss with score 0, distinguishable from a played loss by the presence of the team's ID in `forfeitedTeamIds`.

**05-REQ-028**: Convex shall be the sole authority for every game's status value. Transitions shall be: `not-started → playing` (on successful provisioning, seeding, and invitation acceptance per [05-REQ-032] and [05-REQ-032b]), `playing → finished` (on receipt of the game's terminal state from the SpacetimeDB instance per [05-REQ-038]), and — for tournament games only — `not-started → finished` as a forfeit walkover or no-contest when zero or one Centaur Team's nominated server accepts the game-start invitation per [03-REQ-056] and [05-REQ-032] step 5a. No other transitions shall be permitted, except for the healthcheck-failure rollback described in [05-REQ-036].
...
**Step 6 — Game credential issuance and invitations**. For each participating team, the action calls `issueGameCredential(centaurTeamId, gameId)` from [03] to generate a per-team game credential JWT. The action then sends game invitations via `POST https://{nominatedServerDomain}/.well-known/snek-game-invite` with a `GameInvitationPayload` (Module 03 §4.7) containing the credential, STDB URL, module name, game config, and team roster. Invitations are sent concurrently. The action waits up to 10 seconds for each server's response and partitions teams into `acceptedTeamIds` and `forfeitedTeamIds` based on the responses (a rejection, non-200, or timeout puts the team in `forfeitedTeamIds`).

Branching is determined by `tournamentMode`:
...
**Step 8 — Centaur state initialization**. For each effective participating team, the action calls `initializeGameCentaurState({ gameId, centaurTeamId, snakeIds })` from [06] to set up the Centaur subsystem's game-scoped state.

**Step 9 — Status transition**. A final mutation atomically updates the game record: sets `status` to `"playing"`, writes `stdbInstanceUrl`, `stdbModuleName`, `startedAt`, `boardSeed`, and (for tournament games) `forfeitedTeamIds`. The game is now live. *(See resolved 05-REVIEW-015 and 05-REVIEW-016.)*

**Error handling**: If any step after STDB provisioning fails (other than the structured tournament forfeit branches in step 6), the action tears down the provisioned STDB instance before returning the game to `not-started`. This prevents orphaned instances per 05-REQ-033.
