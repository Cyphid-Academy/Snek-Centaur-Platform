...
**A table arrives with the capability change that fixes its fields, never in advance.** A `defineTable` is not a note about intended state; it is a schema the deployment agrees to and validates every write against. `convex/schema.ts` was empty until `migrate-identity-and-authorization` landed, and what it holds now is exactly that change's own records — `trusted_issuers`, `accepted_assertions`, `sign_in_handoffs`, `platform_admins`, and `system_actions`, the attribution a user is shown — plus `games` and a deliberately partial `centaur_teams`. `centaur_teams` carries **only the field identity reads** (`serverDomain`) and is commented as owned elsewhere: `migrate-team-management` grows it, it does not replace it.

`games` is the one record a game has, and it has grown once already. Identity established `status` and `roster`; `migrate-game-configuration` added the configuration this platform shapes a game with — `config` (the two disjoint halves), `boardPreview` (the single current-preview value, or the structured infeasibility, or `null`), `boardPreviewLocked`, and `boardSeed`. That growth is the rule working, not an exception to it: the capability that fixes a field's meaning adds it to the same record, and `migrate-game-lifecycle` will add launch and succession to this one rather than introducing a second. `users` still does not exist; it arrives with `migrate-accounts-and-profiles`.

**`boardSeed` is returned by no query, and that is a requirement rather than an oversight** (`game-configuration/board-generation-retry`: the seed is accessible to no game client). The configuration queries select the fields they answer with instead of returning the document, so a field added to this table does not leak by default. Keep it that way.

**The gameplay half of `config` is an engine mirror, and it is guarded.** `convex/schema.mirror.test.ts` walks `DEFAULT_RUNTIME_CONFIG`'s leaves against the `gameplayConfig` validator's shape, so a field added to `GameRuntimeConfig` and forgotten here fails the suite. A board-generation field is *not* a mirror failure — the two halves are disjoint by construction, and the same suite asserts they share no leaf name.

The suites in `convex/` run under this package's own `vitest.config.ts` (discovered by the root glob, invoked filtered by `pnpm --filter @cyphid/convex-snek-platform test`). They read the schema's validators as data; nothing here needs `convex-test`, which is the host's business.
...
- `convex/schema.mirror.test.ts` — the engine mirror guard over the gameplay half of a game's configuration
