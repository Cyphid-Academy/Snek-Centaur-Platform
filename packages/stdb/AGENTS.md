...
`migrate-identity-and-authorization` landed the whole connect-time path — `onConnect`, the private `admitted_connection` table, and the four seed tables (`game_binding`, `participant_team`, `roster_member`, `roster_coach`) it validates against. That is not a violation of the carve-out above but the line the carve-out actually draws: **admission is not gameplay.** Who may connect is settled before any rule applies, it is stated in full by `identity-and-authorization` rather than by a change still to be written, and an instance without it is an instance with no access control at all — the one gap that cannot be left open while waiting for its neighbour.

Three consequences to preserve:

- **The seed tables are schema without a writer.** `initialize_game` fills them and belongs to `migrate-game-lifecycle`; do not write it here. Until it exists the tables are empty, no token's issuer matches the empty pin, and the instance admits no one — the safe direction for the gap to fail in. Two obligations travel to that change with them: the pinned issuer is the platform's, and the coach designations it writes must include the platform admins standing at that moment, because `platform-admin-role#implicit-coach-everywhere` is granted at issuance and the instance honours no platform-side role.
- **They carry only the fields admission reads.** The pinned platform issuer and the game id, the participating team ids, and the roster snapshot's two relations — member-to-team, and the coach designations, which are separate because a coach need not be a member of the team they coach. They are not the lifecycle or roster tables in miniature; the change that owns those grows them.
...
The decision itself is pure and lives in `src/admission.ts`, not in the reducer body — deliberately, because `admission-validation#reject-before-touching-state` requires a refused connection to leave no trace, and code that cannot write cannot leave one. Signature verification is the host's: SpacetimeDB checks the JWT and surfaces the claims as `ctx.senderAuth.jwt`, so the module needs no crypto library (`jose` is not a dependency here and must not become one).

**But the host verifies a signature without deciding whose it is, and the module has to.** SpacetimeDB 2.7 resolves verification material by OIDC discovery from the `iss` inside the presented token, and offers no way to bind a host to one issuer — `spacetime start` has `--jwt-pub-key-path` for the host's *own* key and nothing for an issuer allow-list. So a token signed by anyone who publishes a resolvable `/.well-known/openid-configuration` arrives with `hasJWT` true and its signature checked, over claims of their own choosing. That is verified by measurement, not inference. The instance therefore pins the platform's issuer itself: `game_binding.platformIssuer`, seeded once alongside the game id, compared before any other claim. Do not remove that check on the strength of the host having "already verified" the token, and do not resolve anything from a value the token itself carries — that is the whole of `verification-without-shared-secrets#a-valid-signature-from-a-stranger-is-refused` on this substrate.
