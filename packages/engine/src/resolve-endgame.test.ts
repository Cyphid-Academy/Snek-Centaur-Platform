...
import { emptyBoard, makeItem, makeSnake, seed, sid, tid, turn } from "./testkit.js";
import type {
...
function doResolve(
  ...
  return resolveTurn(gameState, stagedMoves, turn(turnNumber), seed(seedN), {
    ...QUIET,
    ...config,
  });
}
...
function redSnake(id: number, x: number): SnakeState {
  return makeSnake({
    snakeId: sid(id),
    body: [
      { x, y: 5 },
...
function blueSnake(id: number, x: number): SnakeState {
  return makeSnake({
    snakeId: sid(id),
...
describe("Phase 7/8 — item spawning (01-REQ-048, 01-REQ-049)", () => {
  ...
  it("spawns nothing at rate 0 and is deterministic for fractional rates", () => {
    ...
    const zero = doResolve(mk(), moves([[0, Direction.Up]]), 1, { foodSpawnRate: 0 });
    expect(eventsOfKind(zero.events, "food_spawned")).toHaveLength(0);
    const a = doResolve(mk(), moves([[0, Direction.Up]]), 1, { foodSpawnRate: 0.5 }, 7);
    const b = doResolve(mk(), moves([[0, Direction.Up]]), 1, { foodSpawnRate: 0.5 }, 7);
  ...
  it("restricts food to fertile cells but lets potions spawn anywhere eligible", () => {
    ...
    const { events } = doResolve(
      ...
      moves([[0, Direction.Up]]),
      1,
      { foodSpawnRate: 1, invulnPotionSpawnRate: 1, invisPotionSpawnRate: 1 },
    );
...
describe("Phase 10 — win conditions (01-REQ-053..058)", () => {
  ...
  it("handles simultaneous elimination on turn 0 with par for everyone (01-REQ-056)", () => {
    const red = { ...redSnake(0, 1), lastDirection: Direction.Left };
    const blue = { ...blueSnake(1, 9), lastDirection: Direction.Right };
    const { outcome } = doResolve(state([red, blue]), new Map(), 0);
    if (outcome.kind !== "draw") throw new Error(`expected draw, got ${outcome.kind}`);
    expect(outcome.scores.get(tid("red"))).toBe(1);
    expect(outcome.scores.get(tid("blue"))).toBe(1);
  });
  ...
  it("does not end before the turn limit (01-REQ-057) and never with maxTurns 0 (01-REQ-058)", () => {
    ...
    expect(doResolve(mk(), m(), 8, { maxTurns: 10 }).outcome.kind).toBe("in_progress");
    expect(doResolve(mk(), m(), 9999, { maxTurns: 0 }).outcome.kind).toBe("in_progress");
  });
...
describe("purity", () => {
  it("does not mutate the input state", () => {
    ...
    doResolve(input, moves([[0, Direction.Up]]), 1, { foodSpawnRate: 1 });
