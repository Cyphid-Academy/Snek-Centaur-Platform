...
5. Sends game invitations to each participating Centaur Team's nominated server domain (per [03]) and waits for responses.
6. Resolves invitations per [03-REQ-056]: for non-tournament games, all servers must accept or the launch aborts; for tournament games, any team whose server rejects or times out forfeits the round, and the launch proceeds with the accepting teams only (≥ 2 acceptances), resolves as a walkover directly to `finished` (1 acceptance), or resolves as a no-contest directly to `finished` (0 acceptances). When the launch proceeds, Convex calls `initialize_game` on the SpacetimeDB instance with the snake roster restricted to accepting teams, updates the game record with the instance URL, and transitions status to `playing`.
...
```
not-started ──[launch + invitations resolved]──► playing ──[game-end + replay persist]──► finished
            └─[tournament: ≤ 1 invitation accepted ⇒ walkover/no-contest]──────────────► finished
```

- `not-started → playing`: Config is frozen (02-REQ-050), board generated by Convex (or locked-in preview used), fresh SpacetimeDB instance provisioned (02-REQ-019, 02-REQ-020), game invitations sent to nominated servers and resolved per [03-REQ-056], then the SpacetimeDB instance is initialised with the pre-computed initial game state, dynamic gameplay parameters, and a snake roster restricted to accepting teams (in tournament games with forfeits, the forfeiting teams' snakes are not spawned).
- `not-started → finished` (tournament-only walkover/no-contest): For tournament games where the game-start invitations resolve with 0 or 1 acceptances, the game transitions directly from `not-started` to `finished` without ever entering `playing`. Any provisioned STDB instance is torn down without `initialize_game` being called. See [03-REQ-056] and [05-REQ-028].
