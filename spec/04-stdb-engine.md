...
**04-REQ-072**: The SpacetimeDB host process — the hosting platform that exposes the management-API surface consumed under [03-REQ-048] and that backs the `POST /v1/database` provisioning call of [05-REQ-032] step 3 — shall expose a **warm-up signal** distinct from per-instance provisioning. When the warm-up signal is received against a host that is in a scaled-to-zero (suspended) state, the host shall transition to a ready state — capable of accepting a subsequent `POST /v1/database` call — and shall return a success response within a budget of **10 seconds** measured from receipt of the signal. The warm-up signal shall not provision a database, shall not mutate the state of any existing database, and shall not require possession of the platform-management JWT consumed under [03-REQ-048]; a lightweight static-token check sufficient to deter casual abuse is permitted in lieu of full management-JWT verification, because the only effect of a successful warm-up call is to resume the host process. A warm-up call against an already-warm host shall be a no-op that returns success within the same budget. *(See resolved 04-REVIEW-022.)*
...
**`board_state`** — Single-row table storing the static board layout. Uses the flat `ReadonlyArray<CellType>` encoding with `y * boardSize + x` indexing per [01] DOWNSTREAM IMPACT note 3.

```typescript
interface BoardStateRow {
  readonly boardSize: number              // edge length in cells
  readonly cells: number[]                // flat array of CellType values, y * boardSize + x indexing
}
```
...
**Design decision — turn-0 clock starts at init completion**: The turn-0 per-turn clock begins ticking when `initialize_game` completes. Convex calls `initialize_game` only after game invitations have resolved ([05-REQ-032], per the forfeit-on-refusal semantics of [03-REQ-056]), so the snake roster passed to `initialize_game` is restricted to the teams whose servers accepted the invitation. The `runtime.clock.firstTurnTimeMs` parameter (default 60000ms per [01-REQ-037]) provides ample time for client connection following `initialize_game` completion. This avoids the complexity of a separate "start game" trigger and is consistent with chess conventions where the clock starts regardless of player readiness.
...
### 2.13 Warm-Up Endpoint

Satisfies 04-REQ-072.

The self-hosted SpacetimeDB host process is deployed on Fly.io under a scale-to-zero hosting model: when no Battle Bunker session is active and no provisioned database is being addressed, Fly.io suspends the host's compute. Resuming the host from suspension incurs a cold-start cost; if that cost were borne synchronously by the per-game `POST /v1/database` provisioning call of [05-REQ-032] step 3, it would visibly delay the `not-started → playing` transition for the first game launched after an idle period. To amortise that cold-start cost away from the game-launch critical path, the host exposes a dedicated warm-up endpoint that Convex calls ahead of provisioning on the schedule of [05-REQ-074]. *(See resolved 04-REVIEW-022.)*

**Endpoint shape**:
- **Method and path**: `POST /v1/warmup` (relative to the host's management-API base URL).
- **Authentication**: `Authorization: Bearer <STDB_WARMUP_TOKEN>` — a static shared secret provisioned alongside the host's management credentials, distinct from the platform-management JWT consumed under [03-REQ-048]. The token check is a lightweight deterrent to casual abuse; an actor in possession of the warm-up token can at most cause the host to wake up, which has bounded cost.
- **Request body**: empty.
- **Response**: `200 OK` with body `{"status":"ready"}` once the host process is up and its management API is bound and ready to accept `POST /v1/database`.

**Behaviour**: On receipt of a warm-up request against a suspended host, the hosting platform resumes the host process; the host responds once its management API surface is ready to serve the subsequent provisioning call. Idempotent — a warm-up call against an already-warm host returns the same response without side effects. The endpoint does not create, enumerate, or mutate any database, and does not require knowledge of any specific `gameId`.
...
interface InitializeGameParams {
  ...
  readonly board: {
    readonly boardSize: number            // edge length in cells
    readonly cells: ReadonlyArray<number>  // CellType values, y * boardSize + x
  }
  readonly snakes: ReadonlyArray<{
    readonly snakeId: number
    readonly letter: string
...
### 3.6 Warm-Up Endpoint Contract

Motivated by 04-REQ-072. Consumed by [05]'s game-configuration-creation path ([05-REQ-074]).

The warm-up endpoint exposed by the self-hosted SpacetimeDB host is the following contract:

- **Method**: `POST`
- **Path**: `/v1/warmup` (relative to the same management-API base URL Convex resolves for [03-REQ-048]).
- **Authentication header**: `Authorization: Bearer <STDB_WARMUP_TOKEN>` — a static shared secret distinct from the platform-management JWT used by `POST /v1/database`.
- **Request body**: empty.
- **Success response**: `200 OK`, body `{"status":"ready"}`.
- **Latency budget**: success response within 10 seconds of receipt for a cold (scaled-to-zero) host; sub-second for an already-warm host.
- **Side effects**: none beyond resuming the host process; no database is created or mutated.

**DOWNSTREAM IMPACT**: [05] must call this endpoint from a Convex action triggered on creation of a new game configuration object ([05-REQ-074]). The endpoint URL is derived from the same Convex environment that supplies the management-API base URL for [03-REQ-048]; the `STDB_WARMUP_TOKEN` is provisioned alongside the host's management credentials.
