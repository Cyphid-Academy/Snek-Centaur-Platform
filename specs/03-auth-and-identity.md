...
- **Exactly one acceptance**: the game shall not enter `playing`. Convex shall transition the game directly from `not-started` to `finished` as a walkover, recording the sole accepting team as the winner with score `1.0` (per [01-REQ-053a] and [01-REQ-054]: `competing_teams = 1`) and the remaining teams as forfeiters with score `0`. No SpacetimeDB instance is needed for the walkover beyond any already provisioned (which shall be torn down).
...
- **1 acceptance**: the round is a walkover. The provisioned SpacetimeDB instance (if any) is torn down without `initialize_game` being called, the sole accepting team is recorded as the winner with score `1.0` (per [01-REQ-053a] / [01-REQ-054]), the other teams are recorded as forfeiters with score `0`, and the game transitions directly from `not-started` to `finished`.
