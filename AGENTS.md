...
| `packages/app-shell/` | `@cyphid/snek-app-shell` | The application shell — what every surface of the one web application shares: the mounting contract a surface is written to, the one state binding it reads through, and the one board rendering. A Svelte 5 library, built by `svelte-package` rather than `tsc`. | 02, 05, 08 |
...
**Testing**: Vitest. Run `pnpm test` across the workspace. Every package should have at least a smoke test confirming it loads. Both SvelteKit apps are excluded from workspace project discovery — their Vite transform conflicts with `@sveltejs/kit` resolution during a workspace run — so `pnpm test` invokes each as a separate filtered run after the main one. A new app needs adding there or its tests will never run. `packages/app-shell` mounts real components too but *is* discovered by the root glob, because the conflict is with SvelteKit and there is none in it: its `vitest.config.ts` is a single project carrying the bare `svelte` plugin and the `browser` resolve condition. Keep it single — the root config discovers project configs, and nested projects are not a thing.
...
**One package is not a `tsc` project and so is named in that script.** `packages/app-shell` ships `.svelte` components, which `tsc` cannot read at all; it is built by `svelte-package` and `build:packages` therefore chains `pnpm --filter @cyphid/snek-app-shell build` after `tsc -b` (in that order — the shell resolves the engine through its `dist/`). That is the exception the rule above tolerates, not a licence to start listing packages: any package that *typechecks* still arrives through the project references alone. For the same reason it is absent from the root `tsconfig.json` references and carries its own `typecheck` script, named in the root one beside the apps'.
...
| `pnpm typecheck` | `build:packages` plus the two foreign TS regimes, the app-shell library and both apps (those three via `svelte-check`, which reads their `.svelte` files as well as their `.ts`) |
...
| `pnpm build:packages` | `tsc -b` over the workspace packages, then `svelte-package` over `packages/app-shell` (their gitignored `dist/`) |
